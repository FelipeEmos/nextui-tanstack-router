...
import { useHref } from "../navigation/router";

function TabsComponent() {
  const hrefBuilder = useHref();
  ...
  ] as const, [hrefBuilder]);

  const matches = useMatches();

  const tabsMatchLevel = 1;
  const selectedKey = matches.length > tabsMatchLevel
...
function RandomNumberRerenderChecker() {
  ...
  return <div className="p-4 bg-slate-300 w-64 rounded-lg flex flex-col gap-4">
    ...
    <Button color="primary" onPress={randomize}>Randomize!</Button>
    <p className="italic">If the Page is being reloaded, then this number will change</p>
  </div>
}
